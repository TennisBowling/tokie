...
pub struct Pretok {
    ...
    /// Maximum digits per number token (0 = unlimited).
    pub max_number_digits: u8,
    /// How letters can be prefixed.
    pub letter_prefix: LetterPrefix,
    /// How letters are scanned (simple vs case-aware).
    pub letter_scanning: LetterScanning,
    /// Where contractions appear (standalone vs suffix).
    pub contraction_position: ContractionPosition,
    /// Whether punctuation can include trailing newlines.
    pub punct_trailing_newlines: bool,
    /// Whether punctuation can include trailing slashes (O200K).
    pub punct_trailing_slash: bool,
    /// Whether space can prefix numbers (GPT-2: true, CL100K/O200K: false).
    pub space_prefix_numbers: bool,
    /// Whether newline sequences use lookahead (GPT-2: true, CL100K/O200K: false).
    ...
    fn next(&mut self) -> Option<Self::Item> {
        ...
        match b {
            b'a'..=b'z' | b'A'..=b'Z' => {
                self.pos += 1;
                self.scan_letters();
                // O200K: contractions are suffixes on words
            ...
            b'\'' => {
                ...
                // For suffix contractions (O200K), apostrophe starts punctuation
                if self.config.contraction_position == ContractionPosition::Standalone {
...
impl PretokIter<'_> {
    ...
    fn handle_space(&mut self, start: usize) {
        ...
        if is_ascii_letter(next_b) {
        ...
        } else if next_b >= b'0' && next_b <= b'9' {
            if self.config.space_prefix_numbers {
                // GPT-2: space + numbers
                self.pos += 2;
                self.scan_numbers(1);
            } else {
                // CL100K: space alone, numbers separate
                self.pos += 1;
            }
        } else if next_b == b' ' || next_b == b'\t' || next_b == b'\r' || next_b == b'\n' {
            // Multiple whitespace
            self.scan_whitespace(start);
        } else if next_b == b'\'' {
            // Space + apostrophe - could be contraction or punctuation
            // For both GPT-2 and CL100K, space + punctuation groups together
            self.pos += 2;
            self.scan_other();
        } else if next_b > b' ' && next_b < 0x80 {
            // Space + other ASCII (punctuation)
            self.pos += 2;
            self.scan_other();
        } else if next_b >= 0x80 {
            // Space + Unicode
            ...
            // Control character or something else
            self.pos += 1;
        }
    }
    ...
    fn handle_other(&mut self, _start: usize) {
        ...
        // Check if this can be a prefix for letters (CL100K style)
        if self.config.letter_prefix == LetterPrefix::NonNewlineNonAlnum && self.pos + 1 < self.len
        {
            let next_b = unsafe { *bytes.get_unchecked(self.pos + 1) };
            if is_ascii_letter(next_b) {
                self.pos += 2;
    ...
    fn handle_unicode(&mut self, start: usize) {
        ...
        match classify_unicode(c) {
            ...
            UnicodeClass::Other => {
                // Check for letter prefix (CL100K)
                    ...
                    && self.pos < self.len
                {
                    ...
                    if is_ascii_letter(next_b) {
                        ...
                        let (nc, nlen) = decode_utf8(unsafe { bytes.get_unchecked(self.pos..) });
                        if is_unicode_letter(nc) {
                            self.pos += nlen;
                            self.scan_letters();
                            // O200K: contractions are suffixes on words
                            if self.config.contraction_position == ContractionPosition::Suffix {
